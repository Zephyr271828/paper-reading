...
5. **Tags** — read `TAGS.md` and select the matching tags from that file only. Add them in YAML frontmatter and as inline `#tags`. Do not create new tags or update `TAGS.md` as part of a paper-card run. After adding the card, append a wiki-link to the relevant tag index files in `tags/` when those index files already exist.
...
- If a selected tag has a matching index file in `tags/`, append the paper wiki-link there. If no matching index file exists yet, leave it alone.
