...
5. Read TAGS.md to find the available tags, then assign only the tags from TAGS.md that fit the paper. Add them in YAML frontmatter (tags: [...]) and as inline #hashtags. Do not create new tags, do not edit TAGS.md, and do not invent tags that are not already listed there. Append a Markdown link like - [method_name](../papers/method_name.md) to each matching tags/*.md index file only when that index file already exists. Do NOT use [[wiki-links]].

6. Add a Related Papers section only if there are related papers already present in papers/. If included, each bullet should be only the bare Markdown link like - [method_name](method_name.md) (relative to papers/) with no extra description. Do NOT use [[wiki-links]]. If there are no related local papers, omit the entire section.
